...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
      ...
      <div class="card card-custom">
        <div class="card-header bg-primary">
          <div class="card-title">
            ...
            <h3 class="card-label text-white">List of Delivered Gifts</h3>
          </div>
          <div class="card-toolbar">
            ...
            <?php if($this->session->userdata('user_role')=='sales_man') { ?>
              ...
              <!--begin::Button-->
              <a data-toggle="modal" data-target="#gift_delivery_add_modal" class="btn btn-pill btn-dark font-weight-bolder">
          ...
          <div class="card-body card-body-custom">
            ...
            <!--begin: Datatable-->
            <table class="table table-header-fixed table-striped table-bordered table-hover table-header-fixed dataTable no-footer" id="gifts_delivered_view_table">
              <thead>
                <tr>
  ...
  <?php $this->load->view('include_modals/gifts_delivery_modals') ?>

  <script type="text/javascript">
    $('.menu-item-active').removeClass('menu-item-active');

    $('#gifts_menu').addClass('menu-item-open menu-item-here');
    $('#gifts_menu_2').addClass('menu-item-active');
  </script>
  ...
  <script type="text/javascript">
    ...
    function fetch_pending_issued_gifts(target) {
      ...
      if(sales_man_id!=' '){
        $.post("<?= base_url() ?>gifts/fetch_pending_gifts_issued",{'sales_man_id':sales_man_id},function(data) {
          ...
          $.each(obj, function(index, value) {
            console.log(value);
            gifts += '<option value="'+value.issue_id+'">'+value.gift_name+'</option>';
          });
          $(target).html(gifts);
        });
      }
    ...
    $('body').on('change', '#gift_delivery_add', function() {
      var issue_id = $('#gift_delivery_add').val();
      if(issue_id!='Select Gift'){
        $.post("<?= base_url() ?>gifts/get_issued_gift_info",{'issue_id':issue_id},function(data) {
          var obj = $.parseJSON(data);
          console.log(obj.gift.balance_quantity);
          $('#gift_delivery_add_qty').attr('max',obj.gift.gift_balance_quantity).attr('min',1);
          $('#gift_delivery_add_qty_info').html('Balance : '+obj.gift.gift_balance_quantity);
        });
      }
    });
    ...
    var gifts_delivered_view_table = $('#gifts_delivered_view_table').DataTable({

     "ajax":{
      url :"<?= base_url().'gifts/select_gifts_delivered' ?>",
      type: "post",
      data: function(d){
    ...
    $('body').on('click', '#gift_delivery_edit_btn', function() {
      var row = $(this).closest('tr').children('td');
      $('#remarks_edit').val(row.eq(5).text());
      $('#doctor_delivery_edit').val(row.eq(11).text()).trigger('change');
      $('#gift_delivery_edit_qty').val(row.eq(3).text());
      $('#gift_delivered_date_edit').val(row.eq(6).text());
      $('#gift_delivery_edit').val(row.eq(2).text());
      var max =  parseFloat(row.eq(12).text());
      $('#gift_delivery_edit_qty').attr('max',max);
      $('#gift_delivery_edit_qty_info').html('Balance : '+max);
    });

    $('body').on('click', '#gift_delivery_delete_btn', function() {
      var row = $(this).closest('tr').children('td');
      $('#delivery_id_delete').val(row.eq(10).text());
      $('#gift_id_delete_delivery').val(row.eq(9).text());
    });
